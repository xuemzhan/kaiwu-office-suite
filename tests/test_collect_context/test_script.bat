...
if "%OUTPUT_FILE%"=="" set "OUTPUT_FILE=runtime\results\context.json"

if not exist "runtime\results" mkdir "runtime\results"

echo [%date% %time%] Starting collect_context test: type=%CONTEXT_TYPE% >> "runtime\logs\collect_context.log"
...
echo [%date% %time%] collect_context test complete >> "runtime\logs\collect_context.log"
