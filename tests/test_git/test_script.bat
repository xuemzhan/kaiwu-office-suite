...
if "%OUTPUT_FILE%"=="" set "OUTPUT_FILE=runtime\results\git_status.txt"

if not exist "runtime\results" mkdir "runtime\results"

echo [%date% %time%] Starting Git status check: repo_path=%REPO_PATH% >> "runtime\logs\git_status.log"
...
echo [%date% %time%] Git status report complete >> "runtime\logs\git_status.log"
