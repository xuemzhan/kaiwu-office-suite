...
if not defined OUTPUT_FILE set "OUTPUT_FILE=results\ocr_result.%OUTPUT_FORMAT%"
echo [%date% %time%] OCR image=%IMAGE_PATH%>>"logs\tesseract_ocr.log"
...
"%TESS%" "%IMAGE_PATH%" "results\temp_ocr" -l "%LANGUAGE%"
...
if /i "%OUTPUT_FORMAT%"=="md" (
  ...
  copy /Y "results\temp_ocr.txt" "%OUTPUT_FILE%" >nul
)
del /q "results\temp_ocr.txt" >nul 2>&1
