...
:fix_shortcuts
set "MENU=%APPDATA%\Microsoft\Windows\Start Menu\Programs\开悟个体增智办公套件"
if not exist "%MENU%" mkdir "%MENU%"
set "KAIWU_MENU=%MENU%"
set "KAIWU_HOME=%CD%"
powershell -NoProfile -ExecutionPolicy Bypass -Command "$w=New-Object -ComObject WScript.Shell; foreach($n in @('系统检测','系统修复')) {$s=$w.CreateShortcut((Join-Path $env:KAIWU_MENU ($n+'.lnk'))); if($n -eq '系统检测') {$s.TargetPath=(Join-Path $env:KAIWU_HOME 'check.bat')} else {$s.TargetPath=(Join-Path $env:KAIWU_HOME 'repair.bat')}; $s.WorkingDirectory=$env:KAIWU_HOME; $s.Save()}"
...
for %%D in (00_Inbox 01_项目资料 02_会议纪要 03_OCR识别 04_个人知识 05_模板库 99_归档) do if not exist "%VAULT%\%%D" mkdir "%VAULT%\%%D"
